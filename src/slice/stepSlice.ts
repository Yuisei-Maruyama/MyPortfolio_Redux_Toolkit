...
export const stepSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchStepsData.pending, (state, _) => { // 非同期実行中、または実行待ち
      state.loading = true
      state.error.status = false
      state.error.message = null
    })
    builder.addCase(fetchStepsData.fulfilled, (state, actions) => { // 実行が正常完了（エラーなし）
      state.loading = false
      state.value = new Array(...actions.payload)
    })
    builder.addCase(fetchStepsData.rejected, (state, _) => { // 実行で何かの異常発生
      state.loading = true
